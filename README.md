...
- 培养配方：管理四阶段边界、参考曲线、通道容差和版本生命周期，支持复制版本；发布新版本时同罐体同编号的旧已发布版本在同一事务内自动废止，旧版本仍被就绪时序或未确认分析引用时整次发布拒绝并返回阻塞清单。
...
| 后端 | Go 1.22、Gin、GORM、validator/v10、JWT |
| 正式数据库 | PostgreSQL 16 |
...
发布新版本（`validated -> published`）在单个数据库事务内完成版本切换闸门：

- 发布前锁定同 `vessel_id + recipe_code` 的版本闸门行与全部旧已发布版本行；数据库部分唯一索引保证每组至多一个 `published` 版本。
- 旧已发布版本仍被 `ready` 状态时序（`ready_series`）或任何未到 `confirmed/voided` 终态的分析（`open_analysis`：queued/analyzing/completed/failed/reviewed/investigating）引用时，整次发布回滚，返回 HTTP 409 `PUBLISH_BLOCKED`，错误信封的 `data` 为阻塞清单（含引用类型、批次号、分析状态、发起人、所属旧版本）。
- 无阻塞时旧版本批量置为 `obsolete`、新版本置为 `published`，两个状态一次生效；任一步失败（含审计写入）全部回滚。
- 闸门 epoch + 行锁保证并发发布或并发发布/废止只有一个成功，负方收到 HTTP 409 冲突并需刷新重试。
- `GET /culture-recipes` 与 `GET /culture-recipes/:id` 在已校验/已发布版本上附带 `publish_gate.blocked` 与 `blockers` 读模型：已校验版本预览发布阻塞，已发布版本展示当前在保引用；前端刷新后与库内状态回读一致。
- 闸门事务、阻塞回滚、终态分析放行、读模型投影与"并发只成功一个"均有 service 层测试覆盖（SQLite WAL 重叠事务）。
